...
Adapter that exposes the FastAPI app defined in :mod:`audio_helper.api`
as MCP tools so any MCP-aware host (agent runtimes, IDE integrations,
custom shells) can call ``convert`` / ``split`` / ``concat`` /
``separate`` / … as first-class tools. Uses :mod:`fastapi_mcp`
...
Then run the MCP server::
    ...
    audio-helper-mcp                 # entry point (see pyproject)
    # or, equivalently:
    python -m audio_helper.mcp
...
try:
    ...
    raise ImportError(
        ...
        "Install with: pip install 'audio-helper[api,mcp]'"
    ) from exc
...
mcp = FastApiMCP(
    ...
    name="audio-helper",
    description=(
        "Audio Helper MCP tools: load, convert, split, concatenate, "
        "silence, room-tone, MFCC similarity, Demucs source separation."
    ),
)
...
def main() -> None:
    """
    Entry point for the ``audio-helper-mcp`` console script.
    ...
    host = os.environ.get("AUDIO_HELPER_HOST", "0.0.0.0")
    port = int(os.environ.get("AUDIO_HELPER_PORT", "8000"))
    # ``log_level`` is intentionally left at uvicorn's default so
    # operators can tune via env / cli. Single worker keeps the demucs
    # separator engine in a shared process (no reload storms across workers).
