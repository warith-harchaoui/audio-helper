...
def load_audio(
    ...
    Decoding goes through :func:`sound_converter` (ffmpeg), NOT libsndfile /
    ``soundfile`` — so formats libsndfile cannot open (AAC/``.m4a``, ``.opus``,
    ``.webm``, and the audio track of video containers) load fine. The
    intermediate float32 WAV is read with :mod:`scipy.io.wavfile`.
    ...
    ------
    ValueError
        If ffmpeg is missing, the file cannot be read/decoded, or it carries
        no audio stream — always with an actionable message.
    """
    osh.checkfile(file_path, msg=f"Input audio file not found: {file_path}")

    # Probe (no decode) for native rate/channels so target_sample_rate=None
    # keeps the original rate and channel count is preserved when asked.
    try:
        probe = ffmpeg.probe(file_path)
    except ffmpeg.Error as exc:
        detail = exc.stderr.decode("utf-8", "replace") if exc.stderr else str(exc)
        raise ValueError(f"ffmpeg could not read {file_path!r}: {detail}") from exc
    except FileNotFoundError as exc:  # ffmpeg binary not on PATH
        raise ValueError(
            "ffmpeg is required to decode audio but was not found on PATH — "
            "install it (e.g. `brew install ffmpeg` / `apt install ffmpeg`)."
        ) from exc
    audio_streams = [s for s in probe.get("streams", []) if s.get("codec_type") == "audio"]
    if not audio_streams:
        raise ValueError(f"no audio stream found in {file_path!r}")
    native_sr = int(audio_streams[0]["sample_rate"])
    native_ch = int(audio_streams[0].get("channels", 1))

    out_rate = int(target_sample_rate) if target_sample_rate else native_sr
    out_channels = 1 if to_mono else (2 if two_channels else native_ch)

    # Transcode ANY input -> float32 WAV via ffmpeg (sound_converter does the
    ...
    # scipy returns (n,) for a single channel, (n, channels) otherwise.
    # Honour the historical contract: mono -> 1-D, two_channels -> (n, 2).
    if to_mono and audio.ndim == 2:
        audio = np.mean(audio, axis=1)
    elif two_channels and audio.ndim == 1:
...
def generate_silent_audio(
    ...
    if ext.lower() == "wav":
        ...
        with osh.temporary_filename(suffix=".wav", mode="wb") as temp_wav:
            wav.write(temp_wav, sample_rate, zeros)
