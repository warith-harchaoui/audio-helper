"""
Smoke test for the MCP surface (``audio_helper.mcp``).
...
def test_mcp_module_imports_and_exposes_app():
    ...
    from audio_helper import mcp as mcp_module

    assert hasattr(mcp_module, "app"), "audio_helper.mcp must re-expose `app`."
    assert hasattr(mcp_module, "mcp"), "audio_helper.mcp must expose the `mcp` handler."


def test_main_entrypoint_is_callable():
    """The ``audio-helper-mcp`` console entry point should be a callable."""
    from audio_helper.mcp import main
