"""
End-to-end integration tests for audio-helper.

These tests download a small JFK speech clip from ``harchaoui.org`` and
exercise every public entry point (load / convert / split / concatenate /
silence / room-tone / MFCC similarity / Demucs source separation). They
require network access and ffmpeg on PATH, so they are marked
``integration`` and excluded from the default ``pytest`` run.

Usage Example
-------------
>>> # Run the full end-to-end suite:
>>> #   pytest -m integration tests/test_audio_helper.py
...
def test_audio_resemblance():
    ...
    assert resemblance > 0.98, (
        f"Audio file should have a resemblance of 1 with itself, got {round(100 * resemblance)}%"
    )
    ...
    assert resemblance > 0.8, (
        f"Audio file should have a high resemblance with a noisy version of itself, got {round(100 * resemblance)}%"
    )
...
def test_conversion():
    ...
    assert is_valid_audio_file(wav), (
        "Converted audio file (mp3 to wav) should be a valid audio file"
    )
    audio, sample_rate = load_audio(wav, to_numpy=True, to_mono=False)
    assert sample_rate == 44100, (
        f"Sample rate of converted audio (mp3 to wav) should match 44100 vs {sample_rate}"
    )

    ogg = osh.join([folder, "converted_audio.ogg"])
    sound_converter(audio_file, ogg, freq=44100, channels=2, overwrite=overwrite)
    assert is_valid_audio_file(ogg), (
        "Converted audio file (mp3 to ogg) should be a valid audio file"
    )
    audio, sample_rate = load_audio(ogg, to_numpy=True, to_mono=False)
    assert sample_rate == 44100, (
        f"Sample rate of converted audio (mp3 to ogg) should match 44100 vs {sample_rate}"
    )
...
def test_chunk():
    ...
    assert is_valid_audio_file(chunk), "Extracted audio chunk should be a valid audio file"
    duration = get_audio_duration(chunk)
    assert duration == end - start, (
        f"Extracted audio chunk duration should match {end - start} vs {duration}"
    )
    ...
    assert error < 1.0, (
        f"Total duration of audio chunks should match the original audio file {total_duration} vs {duration} (error: {error}%)"
    )
    t = durations[:-1]
    mini = np.min(t)
    ...
    error = error1 + error2
    assert error < 1.0, (
        f"Duration of audio chunks should match the split time {split_time} vs min: {mini}, max:{maxi} (error: {error}%)"
    )
    ...
    assert is_valid_audio_file(reconstruction), (
        "Concatenated audio file should be a valid audio file"
    )
    ...
    assert error < 0.01, (
        f"Concatenated audio file should have the same duration as the original audio file {duration} vs {get_audio_duration(audio_file)} (error: {100 * round(error)}%)"
    )
    resemblance = sound_resemblance(reconstruction, audio_file)
    assert resemblance > 0.75, (
        f"Concatenated audio file should have a high resemblance with the original audio file, got {round(100 * resemblance)}%: {reconstruction} and {audio_file}"
    )
...
def test_silent_audio():
    ...
    assert sample_rate == 44100, (
        f"Sample rate of generated silent audio should match 44100 vs {sample_rate}"
    )
    assert len(audio) == duration * sample_rate, (
        f"Generated silent audio should have the correct duration {duration} vs {len(audio) / sample_rate}"
    )
...
def test_separation():
    ...
    assert keys_set == normal_keys_set, (
        f"Separated sources should have the correct keys {keys_set} vs {normal_keys_set}"
    )
    original_signal, sample_rate = load_audio(audio_file, to_numpy=True, to_mono=True)
    ...
    for k in sources:
        ...
        assert error < 1.0, (
            f"Separated source {k} should have the same duration as the original audio file {duration} vs {get_audio_duration(audio_file)}"
        )
    ...
    assert resemblance > 0.75, (
        f"Resemblance between original and reconstructed signals should be high, got {round(100 * resemblance)}% audio_file: {audio_file} vs reconstructed: {mp3_audio_file}"
    )
